...
def test_st_as_de_sexta(page: Page):
    """web — source: as_de_sexta."""
    ...
    # Step 3: select (uf)
    try:
        page.get_by_label('UF').select_option("DF")
    ...
    try:
        page.get_by_label('Edifício').select_option("2")
        page.wait_for_timeout(200)
    except Exception:
        ...
        _fp = {'tag': 'select', 'label': 'Edifício', 'name': 'lstEdificio', 'id': 'lstEdificio', 'text': 'Selecione CNP BRASIL - BRASILIA/DF (17º andar - Ala Oeste) ED. FILIAL I - BRASILIA/DF (SBS Quadra 1) ED. FILIAL II - BRASILIA/DF (SEPN 512) ED. MATRIZ 1 - BRASILIA/DF (SBS Quadra 4) ED. MATRIZ 2 - BRA', 'class_list': ['form-select', 'form-select-sm']}
        _best = resolve_selector(page, _sels, _fp)
        if _best:
            page.select_option(_best, "2")
            page.wait_for_timeout(200)
        else:
