...
_MATRICULA_BR_RE = re.compile(r"\bc\d{5,7}\b", re.IGNORECASE)
...
# Production domains: banco.example.com sem sufixo -des/-tqs/-hom
_PROD_DOMAIN_RE = re.compile(
    r"^(?!.*-(?:des|tqs|hom|dev|homolog)).*\.banco\.example\.com(?:/|$)"
)
...
def detect(
    ...
    for match in _MATRICULA_BR_RE.finditer(s):
        hits.append(
            PiiHit(
                pattern=PiiPattern.MATRICULA_BR,
                value=match.group(0),
                severity=Severity.HIGH,
                context=context,
                match_start=match.start(),
...
def is_production_domain(url: str) -> Optional[PiiHit]:
    ...
    (banco.example.com without -des/-tqs/-hom suffix). None otherwise.
    """
    if not url:
        return None
    try:
