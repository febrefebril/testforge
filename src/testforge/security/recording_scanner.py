...
def scan_recording(rec_dir: str | Path) -> ScanReport:
    ...
    # NB-12: keystroke_buffer.jsonl — alert por fingerprint (value preservado)
    _scan_jsonl_lines(
...
def _handle_keystroke(entry: dict, line_no: int, report: ScanReport, prefix: str):
    """NB-12: alert keystroke_buffer entries by fingerprint (value preserved)."""
    fp = (entry.get("fingerprint") or "").strip()
    key = entry.get("key", "")
    kind = entry.get("kind", "")
    if kind != "char" or not fp:
        return
    ctx = f"keystroke_buffer:{fp}"
    md: dict = {"fingerprint": fp}
    if "password" in fp.lower():
        md["type"] = "password"
        # NB-12: register source even if individual chars don't match PII patterns.
        # The fingerprint itself indicates a password field — that's the alert.
        report.hits_by_source.setdefault(ctx, [])
    hits = detect(key, context=ctx, field_metadata=md)
    if hits:
        report.hits_by_source.setdefault(ctx, []).extend(hits)
