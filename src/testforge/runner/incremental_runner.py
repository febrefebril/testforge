...
class IncrementalRunner:
    ...
    def _write_healing_report(self, out_dir):
        ...
        if self.no_healing:
            ...
            lines.append(f"**Healing DESATIVADO nesta run** (flag `--no-healing` ou default).")
            lines.append("")
            lines.append(f"- Falhas registradas: {failed}")
            lines.append("- Nenhuma tentativa de healing feita.")
            lines.append("")
            lines.append("## Para ativar healing")
            lines.append("- Remover flag `--no-healing`.")
            lines.append("- OU definir env `TESTFORGE_HEALING=1`.")
            lines.append("- OU configurar `.testforge/config.yml` com `healing.enabled: true`.")
            by_family = self._group_failures_by_family()
            if by_family:
                lines.append("")
                lines.append("## Falhas por familia (candidatas a healing)")
                for fam, count in sorted(by_family.items(), key=lambda x: -x[1]):
                    lines.append(f"- {fam}: {count} step(s)")
    ...
    def _group_failures_by_family(self) -> dict[str, int]:
        """REC-15: agrupa falhas por familia para report quando healing off."""
        by_family: dict[str, int] = {}
        for r in self.step_results:
            if r.status != "failed":
                continue
            family = "selector"  # default
            # NB followup: IncrementalStepResult campo real e error_message (nao error)
            error = (getattr(r, "error_message", "") or "").lower()
            if "timeout" in error:
                family = "timeout"
            elif "not found" in error or "not visible" in error:
                family = "selector"
