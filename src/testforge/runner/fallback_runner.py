...
class SmartStepRunner:
    ...
    def execute(self, step_data: dict, strategy: str = "") -> bool:
        ...
        try:
            ...
            if strat == "visibility_wait" or strat == "wait_for_enabled":
                self._adaptive_wait_for_visible(sel)
    ...
    def _adaptive_wait_for_visible(self, sel: str) -> None:
        """RC-15: wait for Angular hydration then element visibility.

        Angular SPA pages may not have rendered the target element yet when
        the step fires — a straight wait_for_selector(visible) times out if
        the DOM is still hydrating. Waiting for networkidle first lets Angular
        finish its initial render cycle before probing element visibility.
        """
        try:
