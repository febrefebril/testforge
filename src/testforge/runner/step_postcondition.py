...
def _normalize_url_for_compare(url: str) -> str:
    """Normalize URL for semantic equality check: sort query params only.

    Fragment is intentionally kept — fragment change (#section) counts as SPA
    navigation. Only query param ordering is normalized to prevent false positive
    passes when the same page reorders params across requests (RC-8).
    """
    if not url:
        return ""
    try:
...
class StepPostconditionValidator:
    ...
    def _validate_click(self, step, next_step, url_before):
        ...
        if causes_navigation:
            ...
            changed = bool(url_before) and _normalize_url_for_compare(url_now) != _normalize_url_for_compare(url_before)
            if changed:
                return PostconditionResult(
                    passed=True,
