...
def _exit_code_from_report(report) -> tuple[int, str]:
    """Nunca presume sucesso quando a evidencia do relatorio e insuficiente."""
    if not isinstance(report, dict): return 2, "indeterminate:report_not_object"
    summary = report.get("summary")
    if not isinstance(summary, dict) or not summary: return 2, "indeterminate:summary_missing"
    required=("passed","healed_validated","healing_rejected","failed","blocked","skipped")
    if any(type(summary.get(k)) is not int or summary[k] < 0 for k in required):
        return 2, "indeterminate:summary_invalid"
    if sum(summary[k] for k in ("healing_rejected","failed","blocked","skipped")):
        return 1, "failure:non_success_steps"
    total, hit = summary.get("asserts_total"), summary.get("asserts_hit")
    if total is not None or hit is not None:
        if type(total) is not int or type(hit) is not int or total < 0 or hit < 0 or hit > total:
            return 2, "indeterminate:assert_summary_invalid"
        if hit < total: return 1, "failure:assert_not_reached"
    if summary["passed"] + summary["healed_validated"] <= 0:
        return 2, "indeterminate:no_confirmed_step"
    return 0, "success:confirmed"
...
def cmd_run_incremental(args):
    ...
    try:
        ...
        exit_code, verdict = _exit_code_from_report(report)
        stream = sys.stdout if exit_code == 0 else sys.stderr
        print(f"[TestForge] verdict={verdict} exit_code={exit_code}", file=stream)
        sys.exit(exit_code)
    except FileNotFoundError as exc:
        print(f"[TestForge] [X] {exc}", file=sys.stderr)
        sys.exit(2)
