...
def _sanitize_name(name: str) -> str:
    ...
    REC-77: normaliza unicode para NFKD antes do regex para garantir
    convencao unica. 'c' → 'c', 'a' → 'a', 'a' → 'a', 'o' → 'o', etc.
    NFKD decompoe caracteres compostos; bytes ASCII resultantes ficam intactos;
    bytes nao-ASCII (combining marks) sao filtrados pelo encode/decode roundtrip.
    """
    ...
    try:
        ...
        if page_title and system not in page_title and page_title not in system:
            print(f"[TestForge] [WARN] metadata.system='{meta.get('system')}' "
                  ...
                  f"Verifique se o sistema esta categorizado corretamente.")
    except Exception:
        pass
...
def _check_duplicate_recording(recordings_root: str, new_raw_events_path: str, new_rid: str) -> None:
    ...
    if not new_hash:
        return
    for existing in _glob.glob(f"{recordings_root}/**/raw_events.jsonl", recursive=True):
        existing_rid = pathlib.Path(existing).parent.name
        if existing_rid == new_rid:
            continue
        if _compute_recording_hash(existing) == new_hash:
            print(f"\n[TestForge] [WARN] REC-71: Esta gravacao parece identica a '{existing_rid}'.")
            print(f"  Sequencia de eventos igual. Considere:")
...
def _warn_old_schema(meta: dict, rec_id: str) -> None:
    ...
    if schema_version < _CURRENT_SCHEMA:
        ...
        if schema_version == 0:
            print(f"  Este recording foi criado antes de 2026-06-27 (pre-fingerprint).")
        print(f"  Fixes de captura lancados apos esta gravacao nao estao presentes.")
        print(f"  Se os resultados forem ruins, considere regravar o cenario.")
...
def _prompt_production_domain(url: str, no_interactive: bool = False) -> bool:
    """BUG-REC-59/86: detecta domínio de PRODUÇÃO CORPORATIVO e alerta QA.
    ...
    print(
        ...
        f"[TestForge] Recomendado: usar ambiente DES/TQS/HOM (URLs *-des.apps.nprd.example.com, *-tqs.example.com).",
        file=sys.stderr,
    )
...
def _run_post_recording_completion(rec_dir: str, rid: str, args,
    ...
    try:
        ...
        # NB-17: read metadata from recording for correct app/base_url
        meta_path = os.path.join(rec_dir, "recording_metadata.json")
        app = args.app or "web"
        ...
        if os.path.exists(meta_path):
            try:
                ...
                app = app or _meta.get("application", app)
                base_url = base_url or _meta.get("base_url", base_url)
            except Exception:
                pass
        stc = normalizer.normalize(rec_dir, f"ST-{rid}", app, base_url)

        checker = IntentCompletenessChecker()
        # NB-17: propaga app + base_url ao completeness report (antes vazios)
        report = checker.check_steps(
            stc.steps, stc.field_values,
            recording_id=rid, application=app, base_url=base_url,
        )
...
def _run_post_recording_validation(rec_dir: str, rid: str, args,
    ...
    if getattr(args, "pilot_mode", False):
        try:
            ...
            # failures em recordings CORPORATIVO (R2/R3/R6*) — QA achou que healing
            ...
            # NB-16: cleanup pilot tmp dir — evita bloat (15KB por gravação)
            try:
                pilot_tmp = os.path.join(rec_dir, "_pilot_tmp")
                if os.path.isdir(pilot_tmp):
                    shutil.rmtree(pilot_tmp, ignore_errors=True)
            except Exception:
                pass
...
def _mark_failed_recording(rec_dir: str, rid: str, reason: str = "validation_failed") -> None:
    ...
    try:
        # NB-09: guard contra _PROJECT_ROOT mal-detectado
        project_root = _PROJECT_ROOT.resolve()
        rec_dir_resolved = pathlib.Path(rec_dir).resolve()
        if not (project_root == rec_dir_resolved or project_root in rec_dir_resolved.parents):
            print(f"[TestForge] [ERRO] rec_dir '{rec_dir}' fora de _PROJECT_ROOT '{project_root}'."
                  f" Failed marker NAO criado para evitar nested recordings_failed/.")
            return
        ...
        # REC-91: create README once explaining this folder
        _readme = failed_root / "README.md"
        if not _readme.exists():
            _readme.write_text("""# recordings_failed/

Esta pasta contem gravacoes que falharam na validacao ou foram marcadas como incompletas.

## Por que um recording acaba aqui?

O pipeline detecta `status: incomplete_intent` (campos sem resolver) e move o recording
para ca automaticamente, preservando todos os artefatos para debugging.

## Como recuperar uma gravacao?

NAO e necessario regravar. Itere sobre o recording existente:

```bash
# Ver o que esta faltando:
testforge compile <recording_id> --check

# Recompilar apos corrigir:
...
Sim, se nao precisar mais dos artefatos. Os recordings em `recordings/` nao sao afetados.
""", encoding="utf-8")
        ...
        # REC-92: prune heavy derived artifacts — keep only essentials
        ...
        # source_dir: relative path only. Windows drive letter stripped (NB-10).
        try:
            rel_source = str(pathlib.Path(rec_dir).resolve().relative_to(project_root))
        except (ValueError, OSError):
            rel_source = pathlib.Path(rec_dir).name

        # NB-10: se ainda tiver Windows drive letter, forcar basename
        if _re.match(r'^[A-Za-z]:', rel_source):
        ...
        marker = {
            ...
            # REC-90: permite link de "esta gravacao foi substituida por <id>"
            "superseded_by": None,
        }
        with open(target / "FAILED_MARKER.json", "w", encoding="utf-8") as f:
            json.dump(marker, f, indent=2)

        # REC-88: recovery hints — QA precisa saber como iterar sem regravar
        print(f"[TestForge] [FAIL] Gravacao marcada como falha: recordings_failed/{target.name}/")
        print(f"\n[TestForge] Para iterar sobre esta gravacao sem regravar:")
        print(f"  testforge compile {rid} --check    # ver o que esta faltando")
        print(f"  testforge compile {rid} --data     # recompilar com dados separados")
        print(f"  (Nao e necessario regravar — itere sobre compile + run)")
...
def _auto_publish_recording(rid: str, rec_dir: str):
    ...
    try:
        ...
        # NB-12: alert when keystroke_buffer indicates password field
        try:
            ...
            if kb_pwd_sources:
                ...
                print(f"  Arquivo sera PRESERVADO localmente para debug (contrato:"
                      f" massa de teste mantida).")
                print(f"  Arquivo NAO sera enviado ao repositorio git (.gitignore + publisher exclude).")
        except Exception:
            pass
...
def _find_recording_leaf_dirs(base_dir: str) -> list[str]:
    ...
    Isso evita confundir pastas agregadoras (ex.: recordings/SIMULADOR) com
...
def cmd_record(args):
    ...
    if not args.url:
        ...
        print("      --name CT-001 --system SIMULADOR --suite credito --test-case CT-001")
        print()
    ...
    with sync_playwright() as pw:
        ...
        # H17: default = "batched" (immediate caused 5-20s overhead on CONSULTA).
        ...
        # REC-71/82: duplicate detection after recording
        _check_duplicate_recording(str(_PROJECT_ROOT / "recordings"), raw_jsonl, rid)
        if _diag:
...
def cmd_compile(args):
    ...
    # Se o usuario apontou para pasta agregadora (ex.: recordings/SIMULADOR),
    ...
    if os.path.exists(meta_path):
        ...
        # REC-64: cross-check metadata.system vs first event page_title
        _warn_system_title_mismatch(meta, raw_events_path)

        # REC-83: alert if recording uses old capture schema
        _warn_old_schema(meta, rec_id)
    ...
    if getattr(args, 'check', False):
        checker = IntentCompletenessChecker()
        # NB-17: propaga rec_id + app + base_url ao completeness report
        report = checker.check_steps(
            stc.steps, stc.field_values,
            recording_id=rec_id, application=app or "", base_url=base_url or "",
        )
        report_dir = os.path.join(rec_dir, "completeness")
...
def _cmd_migrate_uncategorized(args) -> None:
    ...
    import glob as _glob
    recordings_root = _PROJECT_ROOT / "recordings"
    _candidates = []
    if recordings_root.exists():
        _candidates.extend(_glob.glob(str(recordings_root / "*" / "recording_metadata.json")))
    unc_dir = _PROJECT_ROOT / "src" / "testforge" / "uncategorized"
    if unc_dir.exists():
        _candidates.extend(str(p) for p in unc_dir.glob("*/recording_metadata.json"))

    needs_migration = []
    for meta_path in _candidates:
        try:
    ...
    for meta_path, meta in needs_migration:
        ...
        print(f"  Sugestao:  system={inferred_system or '?'} suite=? test_case={rec_id}")
...
def _cmd_link_failed(args) -> None:
    ...
    if not failed_path.exists():
        print(f"[TestForge] FAILED_MARKER nao encontrado: {failed_path}")
        return
...
def _cmd_prune_snapshots(args) -> None:
    """NB-07: remove dom_snapshots/, ax_snapshots/ de uma gravacao."""
...
def main():
    ...
    from testforge import __version__ as _tf_version
    parser = argparse.ArgumentParser(
        description=f"TestForge CLI v{_tf_version} — Gravacao inteligente de testes E2E"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Log detalhado (DEBUG)")
    parser.add_argument("--version", "-V", action="version",
                        version=f"TestForge v{_tf_version}")
    ...
    rec.add_argument("--replay-immediate", action="store_true",
                     help="(Sprint 0) Opt-in para o modo legacy: probes síncronos por evento. Bloqueia overlay no CONSULTA.")
    rec.add_argument("--system", default="",
                     help="Sistema/aplicacao (ex: SIMULADOR). Caminho Git: recordings/{system}/{suite}/{test_case}/")
    ...
    rec.add_argument("--scenario", default="",
                     help="Cenario (4 nivel opcional: system/suite/test_case/scenario)")
    ...
    # publish (Bloco 0: target-aware publication gate)
    pub = sub.add_parser("publish", help="Publica gravacao com target-aware gate (local ou github)")
    pub.add_argument("recording_id", nargs="?", help="ID da gravacao (opcional — usa env var)")
    pub.add_argument("--target", choices=["local", "github"], default="local",
                     help="Destino: 'local' (rede CORPORATIVO, tudo) ou 'github' (publico, so codigo+docs)")
    pub.set_defaults(func=cmd_send)  # reusa infrastructure existente

    # migrate-uncategorized (REC-79: infer taxonomy for old recordings)
    migra = sub.add_parser("migrate-uncategorized", help="Infere system/suite/test_case para recordings sem taxonomy")
    migra.add_argument("--dry-run", action="store_true", help="Lista recordings sem alterar")
    migra.set_defaults(func=_cmd_migrate_uncategorized)

    # link-failed (REC-90: link failed recording to canonical replacement)
    lnkf = sub.add_parser("link-failed", help="Linka um recording falho ao seu substituto canonical")
    lnkf.add_argument("failed_id", help="ID do recording falho (em recordings_failed/)")
    lnkf.add_argument("canonical_id", help="ID do recording canonical que o substituiu")
    lnkf.set_defaults(func=_cmd_link_failed)

    # prune-snapshots (NB-07: remove dom_snapshots/ax_snapshots from a recording)
    prunes = sub.add_parser("prune-snapshots", help="Remove dom_snapshots/ e ax_snapshots/ de uma gravacao")
    prunes.add_argument("recording_id", help="ID da gravacao")
    prunes.set_defaults(func=_cmd_prune_snapshots)
