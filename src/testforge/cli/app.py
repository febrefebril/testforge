    ...
    try:
        ...
        if auto_complete:
            ...
            if os.path.exists(new_report_path):
                with open(new_report_path, encoding="utf-8") as f:
...
def _resolve_publisher():
    """Fonte unica de resolucao do publisher.

    Ordem: testforge_git.yml (perfil publication) > .testforge/config.yml > env vars.
    O perfil canonico vence para que update e publicacao nunca compartilhem
    repositorio por acidente.
    """
    from testforge.publisher import GitPublisher

    try:
        from testforge.publisher.profile_factory import publisher_from_profile

        publisher = publisher_from_profile(_PROJECT_ROOT)
        if publisher is not None:
            logger.info("publisher resolvido por testforge_git.yml (perfil publication)")
            return publisher
    except FileNotFoundError:
        logger.debug("testforge_git.yml ausente; caindo para configuracao legada")
    except Exception as exc:
        logger.warning("testforge_git.yml invalido (%s); caindo para configuracao legada", exc)

    publisher = GitPublisher.from_config() or GitPublisher.from_env()
    if publisher is not None:
        logger.info("publisher resolvido por configuracao legada")
    return publisher


def _publication_log_path() -> str:
    """Arquivo unico de diagnostico de envio, fora da pasta de gravacoes."""
    import os as _os

    base = _os.getenv("LOCALAPPDATA") or _os.path.expanduser("~")
    folder = _os.path.join(base, "TestForge", "logs")
    _os.makedirs(folder, exist_ok=True)
    return _os.path.join(folder, "publication.jsonl")


def _log_publication(record: dict) -> None:
    """Registra uma tentativa de envio em JSONL append-only.

    Sem isto, uma falha de publicacao no ambiente do QA some no terminal e
    ninguem consegue dizer se foi rede, credencial, branch ou configuracao.
    """
    ...
    try:
        ...
        record["testforge_version"] = _get_version_safe()
        path = _publication_log_path()
        with open(path, "a", encoding="utf-8", newline="\n") as handle:
            handle.write(json.dumps(record, ensure_ascii=False, default=str) + "\n")
        logger.info("publicacao registrada em %s", path)
    except Exception as exc:
        logger.warning("nao foi possivel registrar o log de publicacao: %s", exc)
...
def _auto_publish_recording(rid: str, rec_dir: str):
    """Auto-publica artefatos de gravacao no Git se configurado.

    Sempre chamada, mesmo para gravacoes incompletas, para que o time possa
    diagnosticar problemas do TestForge a partir do submission_report.json.
    Toda tentativa, com sucesso ou falha, vira uma linha em publication.jsonl.
    """
    entry = {"event": "auto_publish", "recording_id": rid, "recording_dir": rec_dir}
    try:
        if os.getenv("TESTFORGE_SENSITIVE_REVIEW", "0").strip().lower() in ("1", "true", "yes", "on"):
            print("[TestForge] [SEGURANCA] Auto-publicacao desabilitada. Use 'testforge submit' para revisar e preparar a copia.")
            entry.update(status="skipped", reason="sensitive_review_enabled")
            _log_publication(entry)
            return
        ...
        if publisher is None:
            print(
                ...
                "Preencha testforge_git.yml (perfil publication).",
                file=sys.stderr,
            )
            entry.update(status="skipped", reason="publisher_not_configured")
            _log_publication(entry)
            return
        ...
        if os.path.exists(meta_path):
            ...
            classification = {key: meta.get(key, "") for key in ("system", "suite", "test_case")}
        ...
        if result.success:
            ...
            entry.update(
                ...
                commit=getattr(result, "commit_sha", ""),
                artifacts=len(getattr(result, "artifacts_copied", []) or []),
            )
        else:
            print(f"[TestForge] [WARN] Publicacao falhou: {result.error}", file=sys.stderr)
            print(f"[TestForge]   Diagnostico: {_publication_log_path()}", file=sys.stderr)
            entry.update(status="failed", error=str(getattr(result, "error", "")))
        _log_publication(entry)
    except Exception as exc:
        import traceback
        ...
        print(f"[TestForge]   Diagnostico: {_publication_log_path()}", file=sys.stderr)
        entry.update(status="error", error=str(exc), traceback=traceback.format_exc()[-4000:])
        _log_publication(entry)
...
def _record_qa_wizard(args):
    ...
    # Sistema (mais geral) — so pergunta se nao tiver via flag nem app definido
...
def cmd_doctor(args):
    """Diagnostica o ambiente antes de gravar. Exit code 0 = apto, 1 = bloqueado."""
    from testforge.installer_doctor import format_report, has_blocker, run_doctor

    root = _PROJECT_ROOT
    checks = run_doctor(root, deep=not getattr(args, "quick", False))
    if getattr(args, "json", False):
        ...
        print(json.dumps({"root": str(root), "checks": payload}, ensure_ascii=False, indent=2))
    else:
...
def cmd_demo_heal(args):
    ...
    with sync_playwright() as pw:
        ...
        with open(_PROJECT_ROOT / "recordings/HEAL-DEMO/raw_events.jsonl", encoding="utf-8") as f:
...
def cmd_submit(args):
    ...
    from testforge.submission import (
        DecisionAction, FindingDecision, FindingCategory, SubmissionDecision,
        prepare_submission, scan_recording,
    )
    rec_dir = _PROJECT_ROOT / "recordings" / args.recording_id
    if not rec_dir.is_dir():
        print(f"[TestForge] Gravacao nao encontrada: {rec_dir}", file=sys.stderr)
        raise SystemExit(2)
    scan = scan_recording(rec_dir)
    print(f"[TestForge] Scanner: {len(scan.findings)} ocorrencia(s); {scan.blocking_findings} bloqueante(s).")
    for finding in scan.findings:
        print(f"  {finding.data_type:20} {finding.masked_sample:14} {finding.file_path}:{finding.line} -> {finding.proposed_action.value}")
    decisions=[]
    allowed={item.strip().lower() for item in (args.allow_test_data or "").split(",") if item.strip()}
    for finding in scan.findings:
        if finding.category == FindingCategory.TEST_DATA and finding.data_type in allowed:
            decisions.append(FindingDecision(finding_id=finding.finding_id, action=DecisionAction.PRESERVE,
                confirmed_test_mass=True, justification=args.reason))
    decision=SubmissionDecision(recording_id=args.recording_id,environment=args.environment,cancelled=args.cancel,decisions=decisions)
    report=prepare_submission(rec_dir, _PROJECT_ROOT / "submissions", decision, dry_run=args.dry_run)
    print(report.model_dump_json(indent=2))
    if report.source_sha256_before != report.source_sha256_after:
        print("[TestForge] FALHA: gravacao original foi alterada.",file=sys.stderr); raise SystemExit(2)
    if report.status == "cancelled": raise SystemExit(3)


def cmd_send(args):
    """Re-publica artefatos de gravacao para o repositorio Git configurado."""
    if os.getenv("TESTFORGE_SENSITIVE_REVIEW", "0").strip().lower() in ("1", "true", "yes", "on"):
        print("[TestForge] [BLOQUEADO] send legado desabilitado pelo P0.2-S. Prepare e revise com 'testforge submit'.", file=sys.stderr)
        raise SystemExit(2)
...
def main():
    ...
    # doctor (S1: gate de ambiente antes de gravar)
    doctor_cmd = sub.add_parser(
        "doctor",
        help="Diagnostica o ambiente (Python, tkinter, Git, navegador do sistema, config)",
    )
    doctor_cmd.add_argument("--quick", action="store_true",
                            help="Nao tenta abrir o navegador")
    doctor_cmd.add_argument("--json", action="store_true",
                            help="Saida em JSON para o instalador e para o log")
    ...
    # submit (P0.2-S, never publishes)
    submit = sub.add_parser("submit", help="Revisar dados sensiveis e preparar copia segura; nunca publica no Git")
    submit.add_argument("recording_id")
    submit.add_argument("--dry-run", action="store_true", help="Somente analisa; nao grava nem publica")
    submit.add_argument("--cancel", action="store_true", help="Registra cancelamento; nao grava nem publica")
    submit.add_argument("--environment", default="DES", choices=["DES", "HML", "PRD"])
    submit.add_argument("--allow-test-data", default="", help="Categorias autorizadas neste envio, ex: cpf,cnpj")
    submit.add_argument("--reason", default="", help="Justificativa obrigatoria ao preservar massa")
    submit.set_defaults(func=cmd_submit)

    # send (Git publisher legado; nao chamado por submit)
