...
(function () {
  ...
  window.__tfDraggingOverlay = false;
  window.__tfDragMoved = false;
  window.__tfIgnoreNextClick = false;
  ...
  // Determina se um evento deve ser ignorado pelo recorder,
  // evitando que interações com o próprio overlay do TestForge sejam
  // registradas como passos do teste.
  function _shouldIgnoreRecordingEvent(e) {
    if (
      !e ||
      !e.target
    ) {
    ...
    if (
      window.__tfDragMode &&
      e.target.closest &&
      e.target.closest('#tf-panel')
    ) {
      return true;
    }
    // Recorder overlay / dialogs are control UI, never part of user flow.
    if (
      e.target.closest &&
      e.target.closest('#tf-overlay, #tf-assert-menu, #tf-stop-confirm')
    ) {
      return true;
    }
    if (window.__tfDraggingOverlay) {
      return true;
    }
    // Mouseup after dragging can dispatch a click on the underlying page.
    if (window.__tfIgnoreNextClick) {
      window.__tfIgnoreNextClick = false;
      return true;
    }
    return false;
  ...
  window.addEventListener('click', async function (e) {
    if (_shouldIgnoreRecordingEvent(e)) {
      return;
    }
    var el = e.target;
    if (window.__tfAssertWaiting) {
    ...
    if (_isSubmitTrigger(el)) {
      ...
      try {
        if (window.__tfCriticalEvent) {
          await window.__tfCriticalEvent({
            type: "submit",
            timestamp:
  ...
  async function _sendCriticalCommand(command) {
    try {
      if (window.__tfCommandQueue) {
        window.__tfCommandQueue.push(command);
      }
      if (window.__tfCriticalEvent) {
        await window.__tfCriticalEvent({
          type: "command",
      ...
      console.error("TF critical command failed", ex);
    }
  }

  async function _confirmStop() {
    ...
    if (assertCount === 0) {
      ...
      document.getElementById('tf-stop-yes').onclick = async function () {
        dlg.remove();
        _captureFinalState('user_stop');
        _showStoppingUI();
        //inicio
        await _sendCriticalCommand('STOP');
        //fim
  ...
  document.addEventListener('mousedown', function (e) {
    ...
    window.__tfDragState = {
      dx: e.clientX - rect.left,
      dy: e.clientY - rect.top,
      startX: e.clientX,
      startY: e.clientY
    };
    window.__tfDraggingOverlay = true;
    window.__tfDragMoved = false;
  ...
  document.addEventListener('mousemove', function (e) {
    ...
    var panel = document.getElementById('tf-panel');
    if (!panel) return;
    if (!window.__tfDragMoved) {
      var movedX = Math.abs(e.clientX - window.__tfDragState.startX);
      var movedY = Math.abs(e.clientY - window.__tfDragState.startY);
      if (movedX > 2 || movedY > 2) window.__tfDragMoved = true;
    }
  ...
  document.addEventListener('mouseup', function () {
    ...
    if (window.__tfDragMoved) {
      // Ignore the synthetic click generated when the drag gesture ends.
      window.__tfIgnoreNextClick = true;
    }
    window.__tfDragState = null;
    window.__tfDraggingOverlay = false;
    window.__tfDragMoved = false;
