...
(function() {
  ...
  window.__tfPausedLocal = false;  // NB-04: local mirror for instant visual toggle
  ...
  // Hotfix 22: era 400ms; digitacao humana em campo currencymask (SIMULADOR) pausa
  ...
  // RC-19: skip <script>/<style>/<noscript> when computing element text.
  // Pages with inline JS/CSS (CADASTRO login, GESTAO legacy) leaked script source
  // into div.text signals, producing garbage selectors.
  function _getTextContent(el) {
    if (!el) return '';
    var text = '';
    for (var _i = 0; _i < el.childNodes.length; _i++) {
      var _n = el.childNodes[_i];
      if (_n.nodeType === Node.TEXT_NODE) {
        text += _n.nodeValue || '';
      } else if (_n.nodeType === Node.ELEMENT_NODE) {
        var _tag = (_n.tagName || '').toLowerCase();
        if (_tag === 'script' || _tag === 'style' || _tag === 'noscript') continue;
        text += _getTextContent(_n);
      }
    }
    return text;
  }
  ...
  function _pushEvent(type, el) {
    ...
    var _el = el || document.activeElement;
    ...
    var target = _extractTarget(_el);
    // RC-27: Angular renders id/role async. If both missing, retry once after
    // 100ms so the framework has time to flush attribute updates before commit.
    if (target && !target.element_id && !target.role) {
      setTimeout(function() {
        try { _commit(_extractTarget(_el) || target); } catch(_e) { _commit(target); }
      }, 100);
    } else {
      _commit(target);
    }
  ...
  // ---- Field snapshot ----
  // NB-15: richer fingerprint for fields without id/name
  ...
  function _snapshotFields() {
    ...
    document.querySelectorAll('input, textarea, select').forEach(function(el) {
      ...
      snapshots.push({
        ...
        fingerprint: _fpForField(el, tag),
        identifiers: {
          id: el.id || null,
          name: el.name || null,
  ...
  (function _h21_install_inline_prompt() {
    ...
    document.addEventListener('blur', function(e) {
      ...
      var label = (labelEl && labelEl.textContent && labelEl.textContent.trim())
        ...
        || el.id
        || tag;
      var fingerprint = _fpForField(el, tag);
  ...
  // Covers ng-currency-mask (banco SIMULADOR: el._mask), Inputmask (RobinHerbots),
  ...
  function _extractRawMaskValue(el) {
    ...
    try {
      ...
      // ng-currency-mask (banco SIMULADOR) — el._mask with getRawValue()
      ...
      // Hotfix 22: Angular directive-only mask (ex.: SIMULADOR banco `currencymask=""`
  ...
  function _addStep(action, el, assertType) {
    try {
      ...
      // NB-14: proper key/value serialization (el.attributes is NamedNodeMap)
      if (el.attributes) {
  ...
  // ---- NB-05: Auto-fire blocklist (carousel, swiper, slick, owl) ----
  ...
  window.addEventListener('click', function(e) {
    ...
    if (_isOverlayElement(el)) return;
    // NB-05: skip auto-fire elements (carousel, swiper, slick, owl)
    ...
    if (window.__tfAssertWaiting) {
      ...
      _highlight(el);
      var elAccessibleName = el.getAttribute('aria-label') || el.getAttribute('aria-labelledby') ||
                              el.getAttribute('title') || el.getAttribute('placeholder') ||
                              (el.textContent||'').trim().replace(/\s+/g,' ').substring(0,80);
      // RC-14: warn when element has no accessible identity — assert target likely too broad.
      if (!elAccessibleName) {
        _showToast('[AVISO] Elemento sem nome acessível — verifique se é o alvo correto (Esc cancela)');
      }
      var elDesc = elAccessibleName.substring(0,40) || el.tagName.toLowerCase();
      _showToast('Assert em: "' + elDesc + '" — escolha o tipo');
    ...
    if (_isSubmitTrigger(el)) {
      ...
      // NB-24: dedup rapid-fire submits (< 500ms same target)
      var _submitKey = (el.id||'') + '|' + (el.tagName||'');
      var _now = Date.now();
      window.__tfLastSubmitByKey = window.__tfLastSubmitByKey || {};
      if (window.__tfLastSubmitByKey[_submitKey] && (_now - window.__tfLastSubmitByKey[_submitKey]) < 500) {
        return;
      }
      window.__tfLastSubmitByKey[_submitKey] = _now;
  ...
  function _showOverlay() {
    ...
    try {
      // NB-03: cross-origin — prefers external count injected by Python
    ...
    var ctxHtml = ctxLabel
      ...
      : '';
    var ver = window.__tfVersion || '';
    var versionHtml = ver
      ? '<div style="font-size:9px;color:#64748b;margin-top:1px;text-align:right;letter-spacing:0.05em">v' + ver + '</div>'
      : '';
      ...
      '<div id="tf-panel" style="position:fixed;top:8px;right:8px;background:#1a1a2e;color:#fff;padding:8px 14px;border-radius:8px;font:14px monospace;z-index:99999;display:flex;flex-direction:column;gap:4px;box-shadow:0 4px 16px rgba(0,0,0,0.3)">',
        '<div id="tf-panel-body" style="display:flex;flex-direction:column;gap:4px">',
          '<div style="display:flex;gap:12px;align-items:center">',
            ...
            '<button id="tf-btn-pause" type="button" style="background:#334155;color:#fff;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font:12px monospace" title="Shift+P">||</button>',
            '<button id="tf-btn-stop" type="button" style="background:#991b1b;color:#fff;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font:12px monospace" title="Shift+S">[]</button>',
            '<button id="tf-btn-assert" type="button" style="background:#6366f1;color:#fff;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font:12px monospace" title="Shift+A">Assert</button>',
            '<span style="color:#aaa">|</span>',
    ...
    document.getElementById('tf-btn-pause').onclick = function() {
      window.__tfCommandQueue.push('TOGGLE_PAUSE');
      // NB-04: instant visual toggle Gravando / Pausado
      window.__tfPausedLocal = !window.__tfPausedLocal;
      var status = document.getElementById('tf-status');
      var dot = document.getElementById('tf-dot');
      var btn = document.getElementById('tf-btn-pause');
      if (window.__tfPausedLocal) {
        if (status) status.textContent = 'Pausado';
        if (dot) dot.style.color = '#f59e0b';
        if (btn) { btn.textContent = '0'; btn.style.color = '#ef4444'; btn.style.fontWeight = 'bold'; }
      } else {
        if (status) status.textContent = 'Gravando...';
        if (dot) dot.style.color = '#e94560';
        if (btn) { btn.textContent = '||'; btn.style.color = '#fff'; btn.style.fontWeight = 'normal'; }
      }
    };
  ...
  function _showAssertMenu(x, y) {
    ...
    [
      ...
      {type:'erro_esperado', label:'Error', bg:'#dc2626'},
  ...
  // RC-16: track last-known value per fingerprint — emit ONLY changed fields.
  // Before: every 2s snapshot emitted all form fields (including unchanged
  // siblings like inputSearchNis with value=""), inflating raw_events 5x.
  window.__tfLastFieldSnapshotValues = {};
  window.__tfFieldSnapshotInterval = setInterval(function() {
    try {
      var snaps = _tf_snapshotFields();
      if (!snaps || !snaps.length) return;
      var changed = [];
      for (var _si = 0; _si < snaps.length; _si++) {
        var _s = snaps[_si];
        var _fp = _s.fingerprint;
        if (window.__tfLastFieldSnapshotValues[_fp] !== _s.value) {
          window.__tfLastFieldSnapshotValues[_fp] = _s.value;
          changed.push(_s);
        }
      }
      if (changed.length) {
    ...
    try {
      root.addEventListener("click", function(e) {
        try {
          var el = e.target;
          if (!el) return;
          // NB-01: canonical overlay check — todas variantes tf-* e __tf-*
          if (_isOverlayElement(el)) return;
          if (window.__tfAssertWaiting) return;
          _pushEvent("click", el);
        } catch(_e2) {}
