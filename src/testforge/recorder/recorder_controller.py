...
class RecorderController:
    ...
    def _persist_critical_event(
        self,
        data
    ):
        try:
            logger.info("callback executed payload=%s", data)
            event_type = data.get("type")
            # Critical channel for overlay commands
            if event_type == "command":
                command = data.get("command")
                if command:
                    logger.info("Critical command received: %s", command)
                    self._command_queue.append(command)
                return True
            # critical for submit before redirect
            if event_type == "submit":
                ...
                if target_data:
                    target = TargetInfo(
                        ...
                        all_attributes=target_data.get("attributes")
                        or target_data.get("all_attributes")
                        or {},
                        ...
                        material_field_label=target_data.get(
                            "material_field_label"
                        ),
                    )
                ...
                # Important: append_event, without _capture_snapshots()
                self._store.append_event(event)
                logger.info("Critical submit persisted: %s", data.get("url"))
                return True
            return True
        except Exception as e:
            logger.error("Failed to persist critical event: %s", e)
            return False
    ...
    def flush_events(self):
        ...
        try:
            # inicio 
            for idx, page in enumerate(self._page.context.pages):
                try:
                    state = page.evaluate("""
                    () => ({
                        url: window.location.href,
                        queue: [...(window.__tfCommandQueue || [])]
                    })
                    """)
                    logger.info("PAGE[%s] STATE= %s", idx, state)
                except Exception as ex:
                    logger.info("PAGE[%s] error= %s", idx, ex)
            # fim 
            ...
            # inicio
            if raw_commands:
                logger.info("Raw commands from overlay=%s", raw_commands)
            ...
            if(
                ...
                or "execution context was destroyed" in msg.lower()
                or "most likely because of a navigation" in msg.lower()
            ):
                logger.debug("flush_events tolerated (navigation/page closed): %s", exc)
            else:
    ...
    def handle_commands(self) -> str:
        ...
        # inicio
        logger.info("Pending commands=%s", pending)
        # fim
        for cmd in pending:
            if cmd == "STOP":
                # inicio
                logger.info("Stop command received from overlay")
                # fim
