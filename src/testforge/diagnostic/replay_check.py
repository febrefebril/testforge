...
class ReplayCheck:
    ...
    # recorder thread; on CONSULTA (mat-select expansion → many DOM events) it
    ...
    def __init__(self, page, mode: str = "batched",
        ...
        # RC-6: store (step_id, candidates, capture_url) so drain() can
        # navigate to each step's URL before probing.
    ...
    def check(self, step_id: str, candidates: list) -> Optional[dict]:
        ...
        RC-6: no modo batched, armazena a URL atual com o step para que
        drain() possa navegar de volta ao contexto correto antes de probar.
        """
        if self._mode == "batched":
            try:
                capture_url = self._page.url or ""
            except Exception:
                capture_url = ""
    ...
    def drain(self) -> list[dict]:
        ...
        RC-6: agrupa por URL de captura e navega antes de probar quando
        a URL corrente difere da URL do step.
        """
        out: list[dict] = []
        _nav_timeout_ms = 5000
        for sid, cands, capture_url in self._pending:
            if capture_url:
                try:
                    ...
                    current_url = ""
                if current_url != capture_url:
                    try:
                        self._page.goto(capture_url, timeout=_nav_timeout_ms,
                                        wait_until="domcontentloaded")
                    except Exception as nav_exc:
                        logger.debug("drain: nav to %s failed: %s", capture_url, nav_exc)
                        rec = {
                            "step_id": sid,
                            "ts_checked": datetime.now(timezone.utc).isoformat(),
                            "selector_attempted": self._first_selector(cands),
                            "resolved": False,
                            "elapsed_ms": 0.0,
                            "error": f"url_context_unavailable: {nav_exc}",
                        }
