...
class FrameworkDetector:
    ...
    def _detect_once(self) -> dict:
        """Executa todas as 4 tecnicas uma vez e retorna resultado combinado."""
        evidence: list[str] = []
        flags: dict[str, Any] = {
    ...
    def detect(self) -> dict:
        """Executa _detect_once com retry quando DOM vazio (RC-7)."""
        import time as _time
        result = self._detect_once()
        page_failed = any("page_eval_failed" in e for e in result.get("evidence", []))
        has_bundle = any(e.startswith("bundle[") for e in result.get("evidence", []))
            ...
            for attempt in range(3):
                ...
                if retry.get("primary") != "unknown":
                    retry.setdefault("evidence", []).append(
                        f"framework_retry_succeeded_attempt_{attempt + 1}"
                    )
                    return retry
        return result
