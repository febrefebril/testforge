...
class FrameworkDetector:
    ...
    def detect(self) -> dict:
        ...
        RC-7: retenta ate 3x com 500ms de delay quando DOM vazio e framework unknown.
        """
        result = self._detect_once()
        page_failed = any("page_eval_failed" in e for e in result.get("evidence", []))
        if (result.get("primary") == "unknown"
                and not page_failed
                and result.get("interactive_elements", 0) < 5
                and not any(e.startswith("bundle[") for e in result.get("evidence", []))):
            for attempt in range(1, 4):
                time.sleep(0.5)
                retry = self._detect_once()
                if retry.get("primary") != "unknown":
                    retry.setdefault("evidence", []).append(
                        f"framework_retry_succeeded_attempt_{attempt}"
                    )
                    return retry
                logger.debug("FrameworkDetector retry %d: still unknown", attempt)
        return result

    def _detect_once(self) -> dict:
        """Single detection pass (no retry). Called by detect()."""
        evidence: list[str] = []
        flags: dict[str, Any] = {
