...
class AngularMaterialHandler(ComponentHandler):
    ...
    def normalize(self, steps: list) -> None:
        ...
        self._aggregate_autocomplete(steps)
    ...
    def _dedup_datepicker_sequences(self, steps: list) -> None:
        ...
        - **Click-only completion** (e.g. banco SIMULADOR): the date input has a
        ...
        while i < len(steps):
            ...
            # RC-13: picker echo with valid DD/MM/YYYY → aggregate as select_date.
            # "/" distinguishes "03/01/1984" from garbled raw digits "01011984".
            picker_echo_valid_date = picker_echo_fill and "/" in (steps[found_fill].value or "")

            if picker_echo_valid_date:
                # Calendar clicks completed date selection; picker wrote a valid
                # DD/MM/YYYY value to the input. Aggregate: emit select_date with
                # the captured date, suppress the calendar navigation noise.
                steps[found_fill].action = "select_date"
                ctx = steps[found_fill].context or {}
                ctx["date_from_calendar"] = True
                steps[found_fill].context = ctx
                for k in range(seq_start, found_fill):
                    if not steps[k].skip_reason:
                        steps[k].skip_reason = "datepicker_aggregated"
                i = found_fill + 1
            elif picker_echo_fill:
                # Garbled raw-digit fill (pre-fix f1c1881): no valid date string
                # available → keep calendar click sequence, skip the unusable fill.
                ...
                # Hotfix 20: click-only completion (e.g. SIMULADOR). The last
    ...
    def _aggregate_autocomplete(self, steps: list) -> None:
        """RC-22: aggregate fill + mat-option click into search_and_select.

        Detects pattern: fill on autocomplete input followed within 5 steps by
        a click on a mat-option or aria-role=option element. Converts both steps
        into a single search_and_select action so the compiler can emit fill +
        wait_for_option + click as an atomic sequence.

        Runs before _skip_dead_fills so "dead" fills with short search terms
        (e.g., "t") are still visible and can be aggregated.
        """
        _OPTION_MARKERS = (
            "mat-option",
            "[role=\"option\"]",
        ...
        def _is_autocomplete_fill(step) -> bool:
            if step.action != "fill" or step.skip_reason:
                return False
            if not step.target:
                return False
            cands = getattr(step.target, "candidates", None) or []
            sels = [getattr(c, "selector", "") or "" for c in cands]
            return any(m in sel for sel in sels for m in _AUTOCOMPLETE_FILL_MARKERS)

        def _is_option_click(step) -> bool:
            if step.action != "click" or step.skip_reason:
                return False
            cands = (getattr(step.target, "candidates", None) or []) if step.target else []
            sels = [getattr(c, "selector", "") or "" for c in cands]
            return any(m in sel for sel in sels for m in _OPTION_MARKERS)

        i = 0
        while i < len(steps) - 1:
            step = steps[i]
            if not _is_autocomplete_fill(step):
                i += 1
                continue

            # Look ahead up to 5 steps for option click
            for j in range(i + 1, min(i + 6, len(steps))):
                nxt = steps[j]
                if nxt.skip_reason:
                    continue
                if nxt.action in ("navigation", "fill", "assert"):
                    break
                if not _is_option_click(nxt):
                    continue

                # Found: fill + option click → aggregate
                search_term = step.value or ""
                tgt = nxt.target
                option_text = ""
                if tgt:
                    option_text = ((tgt.text or "") or (tgt.accessible_name or "")).strip()
                nxt_cands = (getattr(tgt, "candidates", None) or []) if tgt else []
                option_sel = next(
                    (getattr(c, "selector", "") for c in nxt_cands if getattr(c, "selector", "")),
                    "",
                )

                step.action = "search_and_select"
                ctx = step.context or {}
                ctx["search_term"] = search_term
                ctx["option_text"] = option_text
                ctx["option_selector"] = option_sel
                step.context = ctx
                step.value = search_term
                step.skip_reason = ""

                nxt.skip_reason = "autocomplete_aggregated"
                break
