...
class PlaywrightCompiler:
    ...
    def _emit_v2_call(self, action: SemanticAction, intent: str, step_filename: str) -> str:
        ...
        if action.action == "set_input_files":
            return f"step.set_input_files(page, intent={intent_lit}, value={value_lit}, candidates_file={file_lit})"
        if action.action == "wait_for_navigation":
            url_lit = _json.dumps(action.value or "", ensure_ascii=False)
            return f"step.wait_for_navigation(page, intent={intent_lit}, url={url_lit})"
        if action.action == "select_date":
            date_lit = _json.dumps(action.value or "", ensure_ascii=False)
            return f"step.fill(page, intent={intent_lit}, value={date_lit}, candidates_file={file_lit})"
        if action.action == "search_and_select":
            term_lit = _json.dumps(action.value or "", ensure_ascii=False)
            return f"step.fill(page, intent={intent_lit}, value={term_lit}, candidates_file={file_lit})"
        ...
        for seg_idx, seg in enumerate(segments):
            ...
            for action in tc.steps[start:end]:
                ...
                if action.action == "navigation":
                    ...
                    step_idx += 1
                    lines.extend(self._gen_set_input_files(action, step_idx))
                elif action.action == "wait_for_navigation":
                    step_idx += 1
                    lines.extend(self._gen_wait_for_navigation(action, step_idx))
                elif action.action == "select_date":
                    step_idx += 1
                    lines.extend(self._gen_select_date(action, step_idx))
                elif action.action == "search_and_select":
                    step_idx += 1
                    lines.extend(self._gen_search_and_select(action, step_idx))
            lines.append("")
    ...
    # "so name comum" e que aparecem em formularios SIMULADOR tipicos (multiplos
    ...
    def _gen_set_input_files(self, action: SemanticAction, idx: int) -> list[str]:
        """RC-11: emite page.set_input_files() para input[type=file].

        value contém apenas o nome do arquivo (sem caminho). Em produção o QA deve
        fornecer o path absoluto via test_data.json ou variável de ambiente.
        """
        pw_expr = self._playwright_locator_expr(action.target)
        css_sels = self._top_css_selectors(action.target)
        filename = self._esc(action.value or "")
        lines = [f"    # Step {idx}: set_input_files ({action.value or 'arquivo'})"]
        sel = pw_expr or (f"page.locator({self._esc(css_sels[0])})" if css_sels else "page.locator('input[type=\"file\"]')")
        lines.append(f"    {sel}.set_input_files({filename})")
        lines.append("    page.wait_for_timeout(300)")
        lines.append("")
        return lines

    def _gen_wait_for_navigation(self, action: SemanticAction, idx: int) -> list[str]:
        """RC-12: emite wait_for_load_state para postback / SSO reload."""
        url = action.value or ""
        lines = [f"    # Step {idx}: wait_for_navigation (postback/SSO)"]
        lines.append("    try:")
        lines.append("        page.wait_for_load_state('networkidle', timeout=8000)")
        if url:
            lines.append(f"        page.wait_for_url({self._esc(url)}, timeout=5000)")
        lines.append("    except Exception:")
        lines.append("        page.wait_for_load_state('domcontentloaded', timeout=5000)")
        lines.append("")
        return lines

    def _gen_select_date(self, action: SemanticAction, idx: int) -> list[str]:
        """RC-13: emite fill para datepicker Material agregado.

        Tenta fill direto com a data DD/MM/YYYY capturada pelo picker.
        Se o input Material rejeitar o fill (binding ativo), o teste falha com
        mensagem clara — neste caso o QA deve replay via navegação de calendar
        ou aguardar implementação de fallback click-based.
        """
        pw_expr = self._playwright_locator_expr(action.target)
        css_sels = self._top_css_selectors(action.target)
        date_val = self._esc(action.value or "")
        label = action.value or "data"
        lines = [f"    # Step {idx}: select_date ({label})"]
        sel = pw_expr or (
            f"page.locator({self._esc(css_sels[0])})" if css_sels
            else "page.locator('.mat-datepicker-input')"
        )
        lines.append("    try:")
        lines.append(f"        {sel}.fill({date_val})")
        lines.append("        page.wait_for_timeout(300)")
        lines.append("    except Exception:")
        lines.append("        page.wait_for_load_state('domcontentloaded', timeout=3000)")
        lines.append(f"        {sel}.fill({date_val})")
        lines.append("        page.wait_for_timeout(300)")
        lines.append("")
        return lines

    def _gen_search_and_select(self, action: SemanticAction, idx: int) -> list[str]:
        """RC-22: emite fill + wait + click para autocomplete search_and_select."""
        pw_expr = self._playwright_locator_expr(action.target)
        css_sels = self._top_css_selectors(action.target)
        ctx = action.context or {}
        term = self._esc(action.value or "")
        option_text = ctx.get("option_text", "")
        option_sel = ctx.get("option_selector", "")
        label = f"'{action.value}' → '{option_text}'" if option_text else action.value or ""
        lines = [f"    # Step {idx}: search_and_select ({label})"]
        input_loc = pw_expr or (
            f"page.locator({self._esc(css_sels[0])})" if css_sels
            else "page.locator('[aria-autocomplete], [role=\"combobox\"]')"
        )
        lines.append(f"    {input_loc}.fill({term})")
        lines.append("    page.wait_for_timeout(500)")
        if option_text:
            ot = self._esc(option_text)
            lines.append(f"    page.locator('mat-option:has-text({ot}), .mat-option:has-text({ot})').first.click(timeout=5000)")
        elif option_sel:
            lines.append(f"    page.locator({self._esc(option_sel)}).click(timeout=5000)")
        lines.append("    page.wait_for_timeout(300)")
        lines.append("")
        return lines
