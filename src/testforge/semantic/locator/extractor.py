...
STRATEGY_WEIGHTS = {
    ...
    # Angular reactive forms: formControlName costuma ser uma âncora estável
    # e mais específica que placeholder genérico, como "dd/mm/aaaa".
    "form_control_name_css": 0.97,
...
class LocatorExtractor:
    ...
    def extract(self, target_data: dict, action: str = "click",
        ...
        if native:
            ...
            candidates.append(self._mk(
                ...
                base_stability=max(stability.get("role", 0.0),
                                   stability.get("test_id", 0.0),
                                   stability.get("label", 0.0),
                                   stability.get("form_control_name", 0.0),
                                   stability.get("material_field_label", 0.0),
        ...
        # 6.1) Angular formControlName
        form_control_name = (
            target_data.get("form_control_name") 
            or target_data.get("formControlName")
            or target_data.get("formcontrolname")
        )
        if form_control_name:
            candidates.append(self._mk(
                strategy="form_control_name_css",
                selector=f'[formcontrolname="{form_control_name}"]',
                base_stability=stability.get("form_control_name", 0.0),
                target_data=target_data,
                intent=intent,
                reason=f"formcontrolname={form_control_name}",
            ))
    ...
    def _mk(
        ...
        logger.info(
            "V2 EXTRACTOR EXECUTADO: strategy=%s selector=%s",
            strategy,
            selector,
        )

        stability_map = attribute_stability(
            target_data
        )
        ...
        accessibility_score = stability_map.get(
            "accessibility_score",
            0.0,
        )
        ...
        return LocatorCandidate(
            ...
            role=target_data.get("role"),
            accessible_name=target_data.get(
                "accessible_name"
            ),
            ax_path=list(
                self._ancestor_roles
            ),
            attributes={
                k: v
                for k, v in (
            ...
            attribute_stability=stability_map,
