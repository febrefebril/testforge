...
def _same_target(a, b) -> bool:
    """Returns True if two steps refer to the same UI element."""
    a_target = getattr(a, "target", None)
    b_target = getattr(b, "target", None)
    # SemanticTarget (has element_id attribute)
    if hasattr(a_target, "element_id"):
        a_id = (a_target.element_id or "") if a_target else ""
        b_id = (b_target.element_id or "") if b_target else ""
        a_name = (a_target.accessible_name or "") if a_target else ""
        b_name = (b_target.accessible_name or "") if b_target else ""
        a_sel = a_target.candidates[0].selector if (a_target and a_target.candidates) else ""
        b_sel = b_target.candidates[0].selector if (b_target and b_target.candidates) else ""
    elif isinstance(a_target, dict):
        # Legacy dict target
        a_id = a_target.get("element_id", "")
        b_id = (b_target.get("element_id", "") if isinstance(b_target, dict) else "")
        a_name = ""
        b_name = ""
        a_sel = a_target.get("css_path", "")
        b_sel = (b_target.get("css_path", "") if isinstance(b_target, dict) else "")
    else:
        return False
    if a_id and b_id:
        return a_id == b_id
    ...
    return bool(a_sel) and a_sel == b_sel
...
class RecordingNormalizer:
    ...
    def normalize(self, recording_dir: str, test_id: str = "",
        ...
        # Hotfix: asserts carregados após a criação dos segmentos podem ficar
        # fora do intervalo usado pelo compiler. Garante que o cenário default
        # cubra todos os passos após o merge.
        if stc.scenario_segments:
            stc.scenario_segments[-1]["end_step_exclusive"] = len(stc.steps)
        else:
            stc.scenario_segments = [{
                "start_step": 0,
                "end_step_exclusive": len(stc.steps),
                "name": "default",
            }] if stc.steps else []
        ...
        logger.info("========== SEMANTIC STEPS ==========")

        for idx, step in enumerate(stc.steps):
            logger.info(
                "[%s] action=%s value=%s",
                idx,
                step.action,
    ...
    def _convert_event(self, raw: dict) -> Optional[SemanticAction]:
        ...
        # Eventos fill com file_upload promovidos para acao file_upload.
        file_upload = raw.get("file_upload")
        if event_type == "fill" and file_upload:
            element_id = (target_data.get("element_id") or target_data.get("id") or "")
            selector = f"#{element_id}" if element_id else "input[type=file]"
            spec_action = {"kind": "file_upload", "selector": selector, "files": file_upload}
            action_obj = SemanticAction(
                action="file_upload",
                target=self._build_target(target_data),
                context={"timestamp": raw.get("timestamp", "")},
                specialized_action=spec_action,
            )
            return action_obj
