...
class RecordingReadinessGate:
    ...
    def evaluate(
        ...
        # NB-23: noise ratio metric — compiled_steps / user_asserts. Ratio > 3
        # indica gravação inflada por lixo (overlay, carousel, auto-fire).
        # Emite warning para o time avaliar qualidade da gravação.
        try:
            user_asserts = 0
            if completeness_report and getattr(completeness_report, "fields", None):
                user_asserts = len(completeness_report.fields)
            compiled_total = report.total_steps or 0
            if user_asserts > 0 and compiled_total > 0:
                ratio = compiled_total / user_asserts
                if ratio > 3.0:
                    report.warnings.append(
                        f"NB-23: high noise ratio {ratio:.1f}x "
                        f"(compiled={compiled_total} / user_intent={user_asserts}). "
                        f"Gravação pode conter ruído (overlay, carousel, auto-fire)."
                    )
        except Exception:
            pass
