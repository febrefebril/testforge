...
# NB-12: arquivos que nunca vao pro repo git compartilhado (contem secrets
# em plaintext OR sao reconstrucao tecla por tecla). Continuam existindo
# localmente em recordings/<id>/ para debug e healing.
_NEVER_PUBLISH_FILENAMES = frozenset({
    "keystroke_buffer.jsonl",
})

# Bloco 0: dois targets suportados.
#   'local' → repo interno rede CORPORATIVO. Inclui gravacoes + massa.
#   'github' → repo publico. APENAS codigo + docs sanitizadas.
class PublishTarget(str):
    LOCAL = "local"
    GITHUB = "github"

# Blocklist para target=github
_GITHUB_BLOCKLIST_GLOBS = [
    "recordings/**",
    "recordings_failed/**",
    "semantic_tests/**",
    "**/raw_events.jsonl",
...
class GitPublisher:
    ...
    def _build_remote_path(self, recording_id: str, metadata: dict) -> str:
        ...
        def _split_segments(value: str) -> list[str]:
            ...
            if not raw:
                return []
            # Accept values pasted as paths (e.g. "/consulta/pesquisa de vagas/")
    ...
    def _generate_submission_report(
        ...
        # NB-22: canonical source is importlib.metadata (pyproject.toml).
        # VERSION file is fallback for backwards compat.
        version = "unknown"
        try:
            from importlib.metadata import version as _importlib_version
            version = _importlib_version("testforge")
        except Exception:
            try:
        ...
        report = {
            ...
            # REC-84: criteria detalhado + failing criteria list
            "criteria": criteria,
            "failing_criteria": [k for k, v in criteria.items() if v is False],
            "steps": steps,
    ...
    def _copy_artifacts(
        ...
        if os.path.isdir(rec_dir):
            for root, dirs, files in os.walk(rec_dir):
                ...
                for fname in files:
                    # NB-12: skip files that must never enter the tracked repo
                    if fname in _NEVER_PUBLISH_FILENAMES:
                        continue
...
"""
    ...
    # Bloco 0.6: publication target gate

    @staticmethod
    def _scan_before_publish(staged_files: list[str], target: str) -> tuple[list[str], list[str]]:
        """Retorna (files_ok, files_blocked_for_target). GitHub filtra artefatos sensiveis."""
        from fnmatch import fnmatch
        if target != PublishTarget.GITHUB:
            return staged_files, []
        blocked = []
        ok = []
        for f in staged_files:
            matched = any(fnmatch(f, g) for g in _GITHUB_BLOCKLIST_GLOBS)
            if matched:
                blocked.append(f)
            else:
                ok.append(f)
    ...
    @staticmethod
    def _alert_and_prompt(blocked: list[str]) -> str:
        print(f"\n[TestForge] ALERTA: {len(blocked)} arquivo(s) sensivel(is) staged para publicacao no GitHub:")
        for f in blocked[:20]:
            print(f"  - {f}")
        if len(blocked) > 20:
            print(f"  ... (+{len(blocked)-20} outros)")
        print("\nContrato: GitHub publico recebe apenas codigo + docs sanitizadas.")
        print("Opcoes:")
        print("  [a] abortar publicacao")
        print("  [u] unstage esses arquivos e continuar (recomendado)")
        print("  [f] forcar publicacao (registrar em log)")
        choice = input("Escolha [a/u/f]: ").strip().lower()
        return choice or "a"
