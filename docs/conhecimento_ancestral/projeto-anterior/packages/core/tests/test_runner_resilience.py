...
class TestPostActionValidator:
    ...
    def test_detects_network_5xx(self):
        v = PostActionValidator()
        result = v._check_network(["500 https://api.example.com/consulta"])
        assert result is not None
    ...
    def test_check_url_no_error(self):
        page = MagicMock()
        page.evaluate.return_value = "https://www.example.com/simulador"
        v = PostActionValidator()
        assert v._check_url(page) is None

    def test_check_url_with_error_indicator(self):
        page = MagicMock()
        page.evaluate.return_value = "https://www.example.com/erro?code=500"
        v = PostActionValidator()
