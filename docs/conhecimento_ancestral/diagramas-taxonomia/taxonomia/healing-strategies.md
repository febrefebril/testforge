...
# 5.13 `llm_selector_inference`

## Finalidade

Usar inferência assistida por LLM para sugerir locator quando heurísticas determinísticas não forem suficientes, mantendo validação obrigatória.
...
- há evidência suficiente no DOM/snapshot;
- heurísticas principais falharam;
- a inferência pode ser validada objetivamente;
- não há risco de inventar intenção.

## Entradas mínimas

- DOM reduzido e sanitizado;
- ação pretendida;
- candidatos;
- erros anteriores;
...
Sugestão de locator, assert ou escopo, acompanhada de justificativa e confiança.

## Critérios de sucesso

- Sugestão validada por execução;
- Evidência anexada;
...
14. llm_selector_inference, somente como fallback validável
15. patch_rejection_policy, quando houver rejeições repetidas
16. flakiness_promotion_policy, antes da promoção final
```
