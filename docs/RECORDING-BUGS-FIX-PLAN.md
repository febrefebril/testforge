...
**Baseado em**: `.planning/bugs-recording-analysis.md` (92 bugs REC catalogados, 49 recordings, 10 sistemas CAIXA)
...
Análise de recordings reais CAIXA (Portal de Massa, SIFAP, SIFEC, SIOPI, SISGH, SIMAX, PLATAFORMA DES, MASSA_DE_TESTE + uncategorized) revelou 92 bugs em recorder/publisher/compiler/runner. **NÃO são bugs individuais isolados** — são **30 root-causes sistêmicos** com muitos sintomas duplicados entre sistemas.
...
| SIFAP | Farmácia Popular | 2 | 07-02 |
| SIFEC | Login CAIXA | 4 | 07-03/06 |
| SIOPI (produção!) | Sim Habitação | 2 | 07-02 |
| SISGH | Gestão Honras | 1 | 07-02 |
| SIMAX | Agendamento massagem | 20 | 07-02/03 |
| uncategorized | 12 sistemas mistos | 12 | 06-24/26 |
| recordings_failed | 3 SIMAX auto-movidos | 3 | 07-03 |
...
**Sintomas**:
- Senhas/PINs plaintext em raw_events (`Sifap101`, `112233`, `01Bola01`, `Caixa123`, `Lpais03`, typing burst com N tentativas)
- CPF do QA (6 identidades distintas: `934.765.700-02`, `539.867.177-49`, `019.493.184-60`, etc)
- 4 matrículas QA (`c891011`, `c891000`, `c897998`, `c892018`)
- CNPJs de terceiros (13 revendedores em `<select>` textContent)
- PII cliente real (`MANUEL M PINTO`, endereço, email, telefone)
...
- Username typing burst: 6 fills para 6 chars (`C → C899 → C8 → C890 → C89001`)
...
**Sintoma**: R2/R3 mesmo domínio `plataforma-des.caixa` → framework_detection.angular_version=null. R1 detecta OK.
...
- SISGH vs SGH (metadata vs page_title)
...
- `application` field ambíguo ("web" vs "SIOPI")
...
- Duplicação `recordings_failed/` E `simax.zip`
...
**Sintoma**: SIFEC login DES + SISGH login TQS legacy têm `<script>` inline no HTML. Recorder captura como div.text.
...
**Sintoma**: SIOPI R7a/b + uncategorized `verifica_regrecao` gravados em `simuladorhabitacao.caixa.gov.br` (**PRODUÇÃO**), não DES/TQS.

**Escopo**: recorder detect production domains (`caixa.gov.br` sem `-des/-tqs`) → alert antes de gravar.

### RC-21 — Agents DOM validation ausente (memory `[[project-siopi-15-run-analysis]]`)
...
- `fill input value="t"` sem target identifier em TODOS 20 SIMAX (dead fill)
...
**Ação**: reportar time infra CAIXA (Task 27 já criada).
...
- Detect base_url em domínio produção (`caixa.gov.br` sem `-des/-tqs/-hom`).
...
tests/unit/security/test_pii_detector_when_matricula_caixa_then_emits_hit.py
...
def test_no_source_masks_pii_values():
    ...
    original_pw = "112233"  # conhecido do anchor
...
- [ ] Regressão: reprocess R5a → password `112233` PRESERVADO + alert emitido
- [ ] Regressão: reprocess R3 → CPF `019.493.184-60` PRESERVADO + alert emitido
...
- [ ] simax duplicados → 1 canonical + versionados
