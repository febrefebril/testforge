...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""TestForge Recorder Freeze Gate v1.1.
...
def search_patterns(base: Path, patterns: dict[str, re.Pattern[str]], root: Path) -> list[str]:
    ...
    for path in iter_text_files(base, {".py", ".js", ".ts", ".json", ".yaml", ".yml"}):
        ...
        for lineno, line in enumerate(text.splitlines(), 1):
            stripped = line.strip()
            # Comentários não representam acoplamento de execução. Ignorar
            # comentários Python/JS/CSS evita falso positivo por documentação,
            # como "PrimeFaces" citado apenas em comentário do overlay.
            ...
            for name, pattern in patterns.items():
                if pattern.search(line):
                    findings.append(f"{rel(path, root)}:{lineno}: {name}: {stripped[:180]}")
    return findings
