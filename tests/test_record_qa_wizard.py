...
class TestWizardPromptsOnlyMissingFields:
    ...
    def test_prompts_for_all_missing_when_all_absent(self):
        args = _make_args()
        with patch("sys.stdin.isatty", return_value=True), \
             patch("testforge.cli.app._load_last_values", return_value={}), \
             patch("testforge.cli.app._save_last_values"), \
             patch("builtins.input", side_effect=[
                 "MeuTeste", "SIOPI", "regressao", "fluxo-pdc",
             ]):
            out = _record_qa_wizard(args)
        # Wizard order: Sistema → Suite → Caso → Nome
        assert out.system == "MeuTeste"
        assert out.app == "MeuTeste"
        assert out.suite == "SIOPI"
        assert out.test_case == "regressao"
        assert out.name == "fluxo-pdc"
