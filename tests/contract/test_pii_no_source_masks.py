...
REC_ANCHORS = [
    ...
    (
        "recordings/anchor-agendamento",
        {
            "value_mutations.jsonl": ["111.111.111-11"],
            "raw_events.jsonl": ["000000", "111.111.111-11"],
        },
    ),
    (
        "recordings/anchor-cadastro",
        {
            "raw_events.jsonl": ["000.000.000-00", "JOAO DA SILVA"],
        },
    ),
]
...
class TestPiiDetectorNeverModifiesValues:
    ...
    def test_report_flags_masking_applied_false(self):
        # Arrange
        rec = Path("recordings/anchor-agendamento")
        if not rec.is_dir():
    ...
    def test_detector_direct_call_preserves_input_exactly(self):
        ...
        cases = [
            ("000.000.000-00", None),
            ("00.000.000/0001-00", None),
            ("senha123", {"type": "password"}),
            ("000000", {"type": "password", "label": "Senha"}),
