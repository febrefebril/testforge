...
def test_q1_replay_produces_masked_value(api, expect_empty):
    ...
    if expect_empty:
        # Documented: fill() on per-instance override stores raw value (not formatted).
        # The mask ignores the formatting on programmatic sets — fill produces
        # the raw string, not the currency-formatted display value.
        assert state["value"] != "10.000,00", (
            f"fill() now formats the value — fixture behavior changed. "
            f"Update the spike doc. Got: {state}"
        )
    else:
        assert state["value"] == "10.000,00", (
