...
class TestNormalizerEmitsMaterialAnchorCandidate:
    ...
    def test_material_anchor_inserted_at_top_when_label_present(self):
        ...
        # Material anchor must be emitted (may not be first due to score rebalancing)
        assert target.candidates, "no candidates emitted"
        mat = next((c for c in target.candidates if c.strategy == "material_form_field"), None)
        assert mat is not None, "material_form_field candidate not found"
        assert 'mat-form-field' in mat.selector
        assert 'mat-label' in mat.selector
        assert 'has-text("Renda mensal")' in mat.selector
        assert mat.score >= 0.80
    ...
    def test_textarea_tag_preserved(self):
        ...
        mat = next((c for c in target.candidates if c.strategy == "material_form_field"), None)
        assert mat is not None, "material_form_field candidate not found"
        assert ' textarea' in mat.selector
