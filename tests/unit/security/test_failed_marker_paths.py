...
class TestFailedMarkerRelativePath:
    ...
    def test_when_recording_outside_project_then_falls_back_to_basename(self, tmp_path, monkeypatch):
        # Arrange
        ...
        assert markers
        marker = json.loads(markers[0].read_text())
        assert marker["source_dir"] == "external_rec", (
            f"expected basename fallback, got {marker['source_dir']!r}"
        )
