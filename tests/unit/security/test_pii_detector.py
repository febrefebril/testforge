...
class TestPiiDetectorCpf:
    def test_when_cpf_formatted_then_emits_hit_and_preserves_value(self):
        ...
        assert hit.value == "019.493.184-60"  # PRESERVED verbatim
    ...
    def test_when_cpf_inside_larger_text_then_matches(self):
        # Arrange
        value = "MANUEL M PINTO CPF: 019.493.184-60 NIS: 000.00000."
        ...
        assert cpf_hits[0].value == "019.493.184-60"
...
class TestPiiDetectorMatricula:
    def test_when_matricula_caixa_pattern_then_emits_hit(self):
        # Arrange
        for matricula in ("c891011", "c891000", "c897998", "c892018", "C89001"):
            # Act
            hits = detect(matricula, context="raw_events.evt_00005")

            # Assert
            m_hits = [h for h in hits if h.pattern == PiiPattern.MATRICULA_CAIXA]
...
class TestPiiDetectorPassword:
    def test_when_type_password_then_emits_critical_regardless_of_content(self):
        ...
        assert pw_hits[0].value == "Sifap101"  # PRESERVED
...
class TestPiiDetectorUrlScan:
    def test_when_keycloak_url_then_emits_hits_per_param(self):
        # BUG-REC-31
        url = (
            "https://login.des.caixa/auth/realms/intranet/protocol/openid-connect/"
...
class TestPiiDetectorProductionDomain:
    @pytest.mark.parametrize(
        "url,expect_hit",
        [
            ("https://simuladorhabitacao.caixa.gov.br/home", True),
            ("https://sifap-frontend-internet-v2-des.apps.nprd.caixa/", False),
            ("https://plataforma-des.caixa/", False),
            ("https://sifap-front-v2-tqs.apps.nprd.caixa/", False),
        ],
        ids=[
            "siopi_producao_detected",
            "sifap_des_ok",
            "plataforma_des_ok",
            "sifap_tqs_ok",
        ],
