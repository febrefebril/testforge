"""Fase 1 — Tracing + captura de snapshot AX do CDP (paralelo ao overlay JS legado).

Testes unitarios para TracingManager, CDPSnapshotter e RecorderController
sob a flag --use-cdp-recorder.
"""
import os
import tempfile
...
class TestRecorderControllerCDPIntegration:
    def test_use_cdp_false_does_not_attach(self):
        ...
        # context.tracing.start NAO DEVE ser chamado
    ...
    def test_use_cdp_true_starts_tracing_and_attaches(self):
        ...
        page.context.tracing.start.assert_called_once()
        # DOM + AX devem estar habilitados na sessao CDP
    ...
    def test_capture_snapshots_saves_ax_snapshot_when_cdp(self):
        ...
        with tempfile.TemporaryDirectory() as tmproot:
            ...
            # Agora alimenta um evento falso pela fila
            ...
            # Verifica se o diretorio ax_snapshots foi usado
            ax_dir = os.path.join(ctrl._store._session_dir, "ax_snapshots")
            # Caminho pode nao existir se nenhum salvamento ocorreu — verifica se o evento salvo tem ax_snapshot_path
