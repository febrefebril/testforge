...
**Novo v0.1.0:** **Diagnostic Mode** — Modo standalone de coleta de telemetria para equipes de QA, com FrameworkDetector, CaptureQualityTracker, ReplayCheck e GherkinWriter em português.

**Novo v0.1.0:** **Architecture v2 (Phases 1-7)** — Playwright tracing + CDP AX-tree capture parallel, v2 LocatorExtractor, LocatorResolver, SQLite intent catalog L0, Pipes & Filters pipeline, zero-dep tracer + dashboard, YAML-driven ComponentResolver.

**Novo v0.1.0:** **Sprint 0 Hotfixes** — 30+ correções no recorder, normalizer, runner, CLI, publisher, diagnostic e healing. Recorder Sprints A-J-M-O-P-Q-R-S: Material anchors, ACCNAME v1.2, rrweb-lite, finder CSS, mask raw value capture. Refactor: overlay JS extraído para `overlay_inject.js`, IntentReconstructor merged no RecordingNormalizer.

**v0.1.0:** Sistema **ComponentHandler** que detecta e executa componentes de UI framework-specific (Angular Material, PrimeFaces, React MUI) de forma determinística, sem depender de healing. Cada handler sabe abrir overlays, selecionar opções, navegar tabs — reduzindo healing L3 em ~40% para componentes cobertos. Pipeline de validação de intenção que detecta campos perdidos, pergunta valores ao QA, valida incrementalmente, e gera relatório de readiness.
...
### Diagnostic Mode (Sprint 0 — v0.1.0)
...
### Component Handler System (v0.1.0)
...
```bash
# 1. Clonar e instalar
git clone <url-do-repositorio> && cd testforge

# Linux: ativar ambiente
source activate.sh
...
testforge --version     # → TestForge v0.1.0
testforge-gui           # → Interface gráfica

# 4. Gravar um fluxo
testforge record http://localhost:8765 --name "meu-teste" \
  --system SIMULADOR --suite regressao --complete
```

Veja [INSTALL.md](docs/USER-GUIDE/INSTALL.md) para instruções detalhadas de instalação em Windows e Linux.
Veja [GRAVAR-FLUXO.md](docs/USER-GUIDE/GRAVAR-FLUXO.md) para o guia completo de gravação.
Veja [GUI-LAUNCHER.md](docs/USER-GUIDE/GUI-LAUNCHER.md) para usar a interface gráfica.

```bash
# Gravar com validação automática (recomendado)
testforge record http://localhost:8765 --name "meu-teste" --validate-before-ready --pilot-mode

# Compilar com massa de dados externa
testforge compile meu-teste --data

# Executar com self-healing
...
```
src/testforge/
├── handlers/       # ComponentHandler system (v0.1.0)
...
│   ├── overlay_inject.js       # JS overlay extraído (v0.1.0)
...
| Documento | Conteúdo |
|-----------|----------|
| [Instalação](docs/USER-GUIDE/INSTALL.md) | Guia de instalação Windows + Linux |
| [Interface Gráfica](docs/USER-GUIDE/GUI-LAUNCHER.md) | Como usar o `testforge-gui` |
| [Gravar Fluxo](docs/USER-GUIDE/GRAVAR-FLUXO.md) | Guia completo: overlay, asserts, fluxo |
| [Arquivos da Gravação](docs/REFERENCIA/RECORDING-FILES.md) | Legenda de todos os artefatos |
| [Glossário](docs/REFERENCIA/GLOSSARY.md) | Termos técnicos explicados |
| [Workflow QA](docs/TUTORIAIS/qa-workflow.md) | Jornada fim-a-fim do QA |
...
| Sprints Recorder (v0.1.0) | A, A2, A3, B, B2, B3, D, F, J, M, O, P, Q, R, S |
